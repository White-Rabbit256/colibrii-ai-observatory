...
## 20-Persona Board (attractiveness lens) — Run #2 results (2026-06-16)
Wow-scores: desktop strong (UI 8.5, frontend 8.2, dataviz 8.2, security 9, legislator 8.5, motion 7.5, WebGL 7.5); weak spots mobile (5.8) + perf/a11y strict (5) + journalist attribution (4) + 1 BLOCK.
**BLOCK (P08, fixed):** DCDemandChart "≈ Japan's entire consumption" annotation was directionally shaky (Japan ~1,000–1,050 TWh ≈ 945, not clearly "more than") → reframed to the verified, unambiguous "2030 · ~3% del consumo mundial" (IEA); hero label softened the same way.
**Fixes applied same pass:** chart axis/label contrast var(--text3)→var(--text2) (P12); Hero3D energy packets enlarged 0.026→0.046 + additive halo, stage glow 0.85→0.95, dpr reduced→1 (P18/P12); GridHero packets now render on mobile (≥360px, was ≥640) (P19); photo credit softened to "Foto: Wikimedia Commons — ver licencia ↗" (no over-claimed CC version) + img width/height + fetchpriority (P10/P12/P04); donut/bar animations 1100→800ms (P13); consequence bridge-caption after the dam pair (P17); FloatingShare button appears after hero, copies UTM link (P16/P20/P06); Reventazón provenance added below.
**Verified-source addition:** Reventazón 305.5 MW, op. 2016, "largest hydropower project in Central America" — hydropower.org / WebSearch 2026-06 (confidence: reported).
**Backlog (needs browser/infra, can't do from sandbox):** self-host the 4 photos to /public for permanence (egress-blocked here); per-photo author + exact CC version on-screen (P10); Act 3/4 micro-interactives — inline tariff picker, ICE 2024/2025 delta toggle (P15); 3D country as recurring watermarked signature across sections (P14); drag-nodes / mobile 3D toggle / scenario hover-sync (P02/P16); lead-form + Vercel custom analytics events (P20); EN native-editor pass.
